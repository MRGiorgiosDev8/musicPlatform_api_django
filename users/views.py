...
def profile_view(request):
    ...
    like_notifications = (
        PlaylistLikeNotification.objects
...
def playlists_page_view(request):
    ...
    if playlist is None:
        playlist = Playlist.objects.create(user=request.user, title='Favorites', tracks=[])
...
def public_user_page_view(request, username):
    ...
    if playlist is None:
        playlist = Playlist.objects.create(user=profile_user, title='Favorites', tracks=[])
    raw_tracks = playlist.tracks if isinstance(playlist.tracks, list) else []
    try:
        tracks = async_to_sync(_enrich_tracks_list_async)(raw_tracks)
    except Exception:
