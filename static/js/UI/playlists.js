const normalizePlaylistArtist = (value) => String(value || '').trim();

const buildTrackRecords = (items = []) =>
  Array.from(items).map((item, idx) => {
    const card = item.querySelector('.track-playlist');
    const artist = normalizePlaylistArtist(card?.dataset?.trackArtist);
...
const computePlaylistView = (trackRecords = [], state = {}, visibleTracksCount = 6) => {
  const sortMode = state.sortMode || 'new';
  ...
  const maxVisible = Math.min(Math.max(0, visibleTracksCount), sorted.length);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const showEmptyStateIfNeeded = () => {
    ...
    if (root.querySelector('.alert.alert-danger.mb-0.opacity-75')) return;
    if (loadMoreContainer) {
      loadMoreContainer.remove();
      loadMoreContainer = null;
    }
    const alert = document.createElement('div');
    alert.className = 'alert alert-danger mb-0 opacity-75';
  ...
  const trackRecords = buildTrackRecords(getTrackItems());
  ...
  const renderTrackPagination = ({ animate = false } = {}) => {
    ...
    clearNoMatchesState();
    const view = computePlaylistView(trackRecords, state, visibleTracksCount);
    ...
    const maxVisible = view.maxVisible;
