...
# 🎵 RubySound.fm
**RubySound.fm** — музыкальный агрегатор на **Django + DRF**, построенный на **асинхронной архитектуре**. Проект объединяет данные из **Last.fm**, **Deezer** и **iTunes**, предоставляет REST API и интерактивный фронтенд с прослушиванием превью треков и анимациями.

## 🚀 Что делает проект

- **Поиск треков** по названию с обогащением обложками и превью из Deezer/iTunes
- **Трендовые артисты** и **актуальные чарты** с фильтрацией по жанру
- **Batch-запросы** к внешним API для минимизации latency
- **Кэширование** на бэкенде и localStorage на клиенте
- **Асинхронная обработка** через `asyncio.gather` + `httpx.AsyncClient`
- **Безопасный рендеринг** на фронтенде — **без `innerHTML`**, только **DOM API** (`createElement`, `textContent`, `appendChild`, `replaceChildren`)
...
- **Оптимизации фронтенда**:
  - `DocumentFragment` + `replaceChildren` для безопасного рендера
  - lazy-loading изображений
  - никакого `innerHTML` — только безопасный DOM API
