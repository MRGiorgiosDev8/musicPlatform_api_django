...
- 💾 **Backup-центр в админке**: `superuser` может создавать, скачивать, восстанавливать и удалять backup-файлы прямо из Django admin. Есть автоочистка старых архивов по лимиту хранения.
...
- То же самое доступно из Django admin для `superuser` через backup-центр
- В админке можно посмотреть список backup-файлов, скачать любой архив, восстановить базу из выбранного файла и удалить старые архивы
- После создания нового backup автоматически применяется автоочистка по лимиту `BACKUP_KEEP_COUNT` (по умолчанию `10`)
- На странице `admin/login` есть кнопка перехода на главную сайта после восстановления базы
- Путь для сохранения можно переопределить через переменную окружения `BACKUP_DIR`

#### Настройки backup
- `BACKUP_DIR` - директория для хранения архивов
- `BACKUP_KEEP_COUNT` - количество последних архивов, которые нужно оставлять
...
1. Откройте Django admin и войдите как `superuser`.
2. Перейдите в backup-центр.
3. Нажмите `Сделать backup`, чтобы создать новый архив.
4. Используйте `Скачать`, `Восстановить` или `Удалить` для выбранного backup-файла.
5. Кнопка `Очистить старые` удаляет все архивы сверх лимита `BACKUP_KEEP_COUNT`.
